...
export class TitanMemoryModel {
  ...
  forward(xTensor, memoryState) {
    return tf.tidy(() => {
      ...
      if (this.useMemoryReplay) {
        this.addToReplayBuffer(x.flatten().arraySync(), memory.flatten().arraySync(), predicted.flatten().arraySync());
      }
      ...
      if (surpriseValue < 0.01) {
        const cacheKey = this.generateCacheKey(x.flatten().arraySync());
        this.cacheMemoryState(cacheKey, dynamicMemory.flatten().arraySync());
      }
      return {
  ...
  getConfig() {
    return {
      ...
      // Use memoryDim consistently
      outputDim: this.memoryDim,
      // Keep outputDim for backward compatibility
  ...
  getLayerMemoryState(layerIndex) {
   ...
   */
  addToReplayBuffer(input, memory, target) {
  ...
  compressMemory(memory) {
    return tf.tidy(() => {
      // Apply compression
      const compressed = tf.add(tf.matMul(memory.reshape([1, this.memoryDim]), this.compressionWeights.transpose()), this.compressionBias).tanh().reshape([Math.floor(this.memoryDim * this.compressionRate)]);
      return compressed;
    });
  }
   ...
   */
  decompressMemory(compressed) {
    return tf.tidy(() => {
      // Apply decompression (approximate inverse)
      const decompressed = tf.matMul(compressed.reshape([1, Math.floor(this.memoryDim * this.compressionRate)]), this.compressionWeights).reshape([this.memoryDim]);
      return decompressed;
    });
  }
   ...
   */
  storeInLongTermMemory(key, memory) {
   ...
   */
  allocateMemoryDynamically(memory, surprise) {
    return tf.tidy(() => {
      const surpriseValue = surprise.dataSync()[0];
  ...
  cleanupCache() {
    ...
    if (this.llmCache.size > 1000) {
      const entries = Array.from(this.llmCache.entries()).sort((a, b) => a[1].timestamp - b[1].timestamp);
